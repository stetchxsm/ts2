import React from "react";
import { useTranslation } from "react-i18next";
import { Link, useLocation } from "react-router-dom";
import { MapPin, Phone, Mail } from "lucide-react";
import { cn } from "@/lib/utils";
import ArabicText from "./ArabicText";

// Custom Social Media Icons
const FacebookIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
  </svg>
);

const InstagramIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
  </svg>
);

const XIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
  </svg>
);

const WhatsAppIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.465 3.488"/>
  </svg>
);

const Footer = () => {
  const { t, i18n } = useTranslation();
  const location = useLocation();
  const isRTL = i18n.dir() === 'rtl';

  // Helper function to handle navigation
  const handleNavigation = (href: string) => {
    if (href.startsWith('#') && location.pathname === '/') {
      const element = document.querySelector(href);
      if (element) {
        const offset = window.innerWidth < 768 ? 100 : 80;
        window.scrollTo({
          top: element.getBoundingClientRect().top + window.pageYOffset - offset,
          behavior: 'smooth'
        });
      }
    }
  };

  const quickLinks = [
    { name: t('navigation.home'), href: '/' },
    { name: t('navigation.services'), href: '#services' },
    { name: t('navigation.contact'), href: '/contact' }
  ];

  const services = [
    { name: t('navigation.travel'), href: '#services' },
    { name: t('navigation.education'), href: '#services' },
    { name: t('navigation.trade'), href: '#services' }
  ];

  const socialLinks = [
    { icon: FacebookIcon, href: '#', label: 'Facebook' },
    { icon: InstagramIcon, href: '#', label: 'Instagram' },
    { icon: XIcon, href: '#', label: 'X' },
    { icon: WhatsAppIcon, href: '#', label: 'WhatsApp' }
  ];

  return (
    <footer className="bg-gray-900 text-white" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="container px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Company Info */}
          <div className="lg:col-span-1">
            <div className={cn(
              "flex items-center mb-4",
              isRTL ? "justify-start" : "justify-start"
            )}>
              <img
                src="/DWLOGO.png"
                alt={t('app.title')}
                className="h-20 w-auto filter brightness-0 invert"
              />
            </div>

            {isRTL ? (
              <ArabicText
                as="p"
                className="text-gray-400 mb-6 leading-relaxed"
                weight="normal"
                size="md"
              >
                {t('footer.description')}
              </ArabicText>
            ) : (
              <p className="text-gray-400 mb-6 leading-relaxed">
                {t('footer.description')}
              </p>
            )}

            <div className={cn(
              "flex gap-4",
              isRTL ? "justify-start" : "justify-start"
            )}>
              {socialLinks.map((social, index) => {
                const IconComponent = social.icon;
                const getHoverColor = (label: string) => {
                  switch (label) {
                    case 'Facebook': return 'hover:bg-blue-600';
                    case 'Instagram': return 'hover:bg-gradient-to-r hover:from-purple-500 hover:to-pink-500';
                    case 'X': return 'hover:bg-black';
                    case 'WhatsApp': return 'hover:bg-green-500';
                    default: return 'hover:bg-blue-600';
                  }
                };

                return (
                  <a
                    key={index}
                    href={social.href}
                    aria-label={social.label}
                    className={cn(
                      "w-12 h-12 bg-gray-800 rounded-full flex items-center justify-center transition-all duration-300 transform hover:scale-110 hover:shadow-lg",
                      getHoverColor(social.label)
                    )}
                  >
                    <IconComponent />
                  </a>
                );
              })}
            </div>
          </div>

          {/* Quick Links */}
          <div>
            {isRTL ? (
              <ArabicText
                as="h3"
                className="text-lg font-semibold mb-4"
                weight="semibold"
                size="lg"
              >
                {t('footer.quickLinks')}
              </ArabicText>
            ) : (
              <h3 className="text-lg font-semibold mb-4">{t('footer.quickLinks')}</h3>
            )}

            <ul className="space-y-3">
              {quickLinks.map((link, index) => (
                <li key={index}>
                  {link.href.startsWith('#') ? (
                    isRTL ? (
                      <ArabicText
                        as="a"
                        href={link.href}
                        className="text-gray-400 hover:text-white transition-colors duration-300 cursor-pointer"
                        weight="normal"
                        size="md"
                        onClick={(e) => {
                          e.preventDefault();
                          handleNavigation(link.href);
                        }}
                      >
                        {link.name}
                      </ArabicText>
                    ) : (
                      <a
                        href={link.href}
                        className="text-gray-400 hover:text-white transition-colors duration-300"
                        onClick={(e) => {
                          e.preventDefault();
                          handleNavigation(link.href);
                        }}
                      >
                        {link.name}
                      </a>
                    )
                  ) : (
                    isRTL ? (
                      <ArabicText
                        as={Link}
                        to={link.href}
                        className="text-gray-400 hover:text-white transition-colors duration-300"
                        weight="normal"
                        size="md"
                      >
                        {link.name}
                      </ArabicText>
                    ) : (
                      <Link
                        to={link.href}
                        className="text-gray-400 hover:text-white transition-colors duration-300"
                      >
                        {link.name}
                      </Link>
                    )
                  )}
                </li>
              ))}
            </ul>
          </div>

          {/* Services */}
          <div>
            {isRTL ? (
              <ArabicText
                as="h3"
                className="text-lg font-semibold mb-4"
                weight="semibold"
                size="lg"
              >
                {t('footer.services')}
              </ArabicText>
            ) : (
              <h3 className="text-lg font-semibold mb-4">{t('footer.services')}</h3>
            )}

            <ul className="space-y-3">
              {services.map((service, index) => (
                <li key={index}>
                  {isRTL ? (
                    <ArabicText
                      as="a"
                      href={service.href}
                      className="text-gray-400 hover:text-white transition-colors duration-300 cursor-pointer"
                      weight="normal"
                      size="md"
                      onClick={(e) => {
                        e.preventDefault();
                        handleNavigation(service.href);
                      }}
                    >
                      {service.name}
                    </ArabicText>
                  ) : (
                    <a
                      href={service.href}
                      className="text-gray-400 hover:text-white transition-colors duration-300"
                      onClick={(e) => {
                        e.preventDefault();
                        handleNavigation(service.href);
                      }}
                    >
                      {service.name}
                    </a>
                  )}
                </li>
              ))}
            </ul>
          </div>

          {/* Contact Info */}
          <div>
            {isRTL ? (
              <ArabicText
                as="h3"
                className="text-lg font-semibold mb-4"
                weight="semibold"
                size="lg"
              >
                {t('footer.contact')}
              </ArabicText>
            ) : (
              <h3 className="text-lg font-semibold mb-4">{t('footer.contact')}</h3>
            )}

            <div className="space-y-4">
              <div className={cn(
                "flex items-start gap-3",
                isRTL ? "flex-row text-right" : "flex-row text-left"
              )}>
                {isRTL ? (
                  <>
                    <ArabicText
                      as="span"
                      className="text-gray-400 flex-1"
                      weight="normal"
                      size="md"
                      align="right"
                    >
                      القاهرة، مصر
                    </ArabicText>
                    <MapPin className="w-5 h-5 text-gray-400 mt-0.5 flex-shrink-0" />
                  </>
                ) : (
                  <>
                    <MapPin className="w-5 h-5 text-gray-400 mt-0.5 flex-shrink-0" />
                    <span className="text-gray-400">Cairo, Egypt</span>
                  </>
                )}
              </div>

              <div className={cn(
                "flex items-center gap-3",
                isRTL ? "flex-row" : "flex-row"
              )}>
                {isRTL ? (
                  <>
                    <div className="flex-1" style={{ textAlign: 'right', direction: 'rtl' }}>
                      <span className="text-gray-400" style={{ direction: 'ltr', unicodeBidi: 'embed', display: 'inline-block' }}>+20 123 456 7890</span>
                    </div>
                    <Phone className="w-5 h-5 text-gray-400 flex-shrink-0" />
                  </>
                ) : (
                  <>
                    <Phone className="w-5 h-5 text-gray-400 flex-shrink-0" />
                    <span className="text-gray-400" dir="ltr">+20 123 456 7890</span>
                  </>
                )}
              </div>

              <div className={cn(
                "flex items-center gap-3",
                isRTL ? "flex-row" : "flex-row"
              )}>
                {isRTL ? (
                  <>
                    <div className="flex-1" style={{ textAlign: 'right', direction: 'rtl' }}>
                      <span className="text-gray-400" style={{ direction: 'ltr', unicodeBidi: 'embed', display: 'inline-block' }}>info@drwhitegroup.com</span>
                    </div>
                    <Mail className="w-5 h-5 text-gray-400 flex-shrink-0" />
                  </>
                ) : (
                  <>
                    <Mail className="w-5 h-5 text-gray-400 flex-shrink-0" />
                    <span className="text-gray-400" dir="ltr">info@drwhitegroup.com</span>
                  </>
                )}
              </div>
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-gray-800 mt-12 pt-8 pb-4">
          <div className="flex justify-center">
            {/* Copyright Text - Always Centered */}
            {isRTL ? (
              <ArabicText
                as="p"
                className="text-gray-400 text-sm text-center leading-relaxed px-4"
                weight="medium"
                size="sm"
                align="center"
                style={{
                  lineHeight: '1.6',
                  letterSpacing: '0.025em',
                  wordSpacing: '0.1em'
                }}
              >
                {t('footer.rights')}
              </ArabicText>
            ) : (
              <p className="text-gray-400 text-sm text-center leading-relaxed px-4" style={{ letterSpacing: '0.025em' }}>
                {t('footer.rights')}
              </p>
            )}
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
